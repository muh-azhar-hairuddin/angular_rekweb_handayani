...
class Kepala_data_kohort extends CI_Controller {
	...
	public function index()
	{
		...
		if($cek=='login' && $level=='kp'){
			$this->load->view('kepala/data_kohort/view');
		}else {
	...
	function get_data_kohort()
	{
		...
		foreach ($list as $dt) {
			...
			<a class="dropdown-item detail" href="#" onclick="loadPage(\''.'kepala_data_kohort/detail?id_register='.$dt->id_register.'\');" ><i class="fa fa-eye"></i> View</a>
			</div>
			</div>
			</center>';
	...
	public function add()
	{
		...
		if($cek=='login' && $level=='kp'){
			...
			$this->load->view('kepala/data_kohort/add',$d);
		}else {
			redirect(base_url().'logout');
		}
	...
	public function edit()
	{
		...
		if($cek=='login' && $level=='kp'){
			...
			$this->load->view('kepala/data_kohort/edit',$d);
		}else {
			redirect(base_url().'logout');
		}
	...
	public function detail()
	{
		...
		if($cek=='login' && $level=='kp'){
			...
			$d['id_register']=$this->input->get('id_register');
			$d['image_show']=$img;
			$this->load->view('kepala/data_kohort/detail',$d);
		}else {
			redirect(base_url().'logout');
		}
