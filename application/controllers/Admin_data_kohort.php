...
class Admin_data_kohort extends CI_Controller {
	...
	public function index()
	{
		...
		if($cek=='login' && $level=='a'){
			$this->load->view('admin/data_kohort/view');
		}else {
	...
	function get_data_kohort()
	{
		...
		foreach ($list as $dt) {
			...
			<a class="dropdown-item hapus" href="#" data-id="'.$dt->id_register.'"><i class="fa fa-trash"></i> Delete</a>
			<a class="dropdown-item edit" href="#" onclick="loadPage(\''.'admin_data_kohort/edit?id_register='.$dt->id_register.'\');" ><i class="fa fa-pencil"></i> Edit</a>
			<a class="dropdown-item detail" href="#" onclick="loadPage(\''.'admin_data_kohort/detail?id_register='.$dt->id_register.'\');" ><i class="fa fa-eye"></i> View</a>
			</div>
			</div>
			</center>';
	...
	public function add()
	{
		...
		if($cek=='login' && $level=='a'){
			...
			$this->load->view('admin/data_kohort/add',$d);
		}else {
			redirect(base_url().'logout');
		}
	...
	public function edit()
	{
		...
		if($cek=='login' && $level=='a'){
			...
			$this->load->view('admin/data_kohort/edit',$d);
		}else {
			redirect(base_url().'logout');
		}
	...
	public function detail()
	{
		...
		if($cek=='login' && $level=='a'){
			...
			$d['id_register']=$this->input->get('id_register');
			$d['image_show']=$img;
			$this->load->view('admin/data_kohort/detail',$d);
		}else {
			redirect(base_url().'logout');
		}
